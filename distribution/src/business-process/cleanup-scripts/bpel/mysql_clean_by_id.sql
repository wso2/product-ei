...
CREATE PROCEDURE cleanInstance(id BIGINT, inst_state SMALLINT)
BEGIN
        SELECT(' Start deleting instance data with instance ids ');
	START TRANSACTION;
	CREATE TABLE TEMP_CLEAN_INS AS SELECT ID FROM ODE_PROCESS_INSTANCE WHERE INSTANCE_STATE = inst_state AND ID=id;
