...
CREATE PROCEDURE cleanInstance(id BIGINT, inst_state SMALLINT, lastActive DATETIME)
BEGIN
        SELECT(' Start deleting instance data with instance ids ');
	START TRANSACTION;
	CREATE TABLE TEMP_CLEAN_INS AS SELECT ID FROM ODE_PROCESS_INSTANCE WHERE INSTANCE_STATE = inst_state AND LAST_ACTIVE_TIME < lastActive AND ID=id;
...
CALL cleanInstance(@ID, @INST_STATE, @LAST_ACTIVE);
