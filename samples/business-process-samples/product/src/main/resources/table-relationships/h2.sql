...
CREATE TABLE ODE_XML_DATA (XML_DATA_ID BIGINT NOT NULL, DATA CLOB, IS_SIMPLE_TYPE BIT, NAME VARCHAR(255), SCOPE_ID BIGINT, PRIMARY KEY (XML_DATA_ID));
...
CREATE TABLE ODE_SCOPE (SCOPE_ID BIGINT NOT NULL, MODEL_ID INTEGER, SCOPE_NAME VARCHAR(255), SCOPE_STATE VARCHAR(255), PROCESS_INSTANCE_ID BIGINT, PARENT_SCOPE_ID BIGINT, PRIMARY KEY (SCOPE_ID), FOREIGN KEY (PROCESS_INSTANCE_ID) REFERENCES ODE_PROCESS_INSTANCE(ID) ON DELETE CASCADE);
...
CREATE TABLE ODE_CORRELATION_SET (CORRELATION_SET_ID BIGINT NOT NULL, CORRELATION_KEY VARCHAR(255), NAME VARCHAR(255), SCOPE_ID BIGINT, PRIMARY KEY (CORRELATION_SET_ID), FOREIGN KEY (SCOPE_ID) REFERENCES ODE_SCOPE (SCOPE_ID) ON DELETE CASCADE);
CREATE TABLE ODE_CORRELATOR (CORRELATOR_ID BIGINT NOT NULL, CORRELATOR_KEY VARCHAR(255), PROC_ID BIGINT, PRIMARY KEY (CORRELATOR_ID), FOREIGN KEY (PROC_ID) REFERENCES ODE_PROCESS (ID) ON DELETE CASCADE);
CREATE TABLE ODE_CORSET_PROP (ID BIGINT NOT NULL, CORRSET_ID BIGINT, PROP_KEY VARCHAR(255), PROP_VALUE VARCHAR(255), PRIMARY KEY (ID), FOREIGN KEY (CORRSET_ID) REFERENCES ODE_CORRELATION_SET (CORRELATION_SET_ID) ON DELETE CASCADE);
CREATE TABLE ODE_EVENT (EVENT_ID BIGINT NOT NULL, DETAIL VARCHAR(255), DATA BLOB, SCOPE_ID BIGINT, TSTAMP TIMESTAMP, TYPE VARCHAR(255), INSTANCE_ID BIGINT, PROCESS_ID BIGINT, PRIMARY KEY (EVENT_ID), FOREIGN KEY (INSTANCE_ID) REFERENCES ODE_PROCESS_INSTANCE(ID) ON DELETE CASCADE, FOREIGN KEY (SCOPE_ID) REFERENCES ODE_SCOPE (SCOPE_ID) ON DELETE CASCADE, FOREIGN KEY (PROCESS_ID) REFERENCES ODE_PROCESS (ID) ON DELETE CASCADE, FOREIGN KEY (INSTANCE_ID) REFERENCES ODE_PROCESS_INSTANCE(ID) ON DELETE CASCADE);
...
CREATE TABLE ODE_MESSAGE_EXCHANGE (MESSAGE_EXCHANGE_ID VARCHAR(255) NOT NULL, CALLEE VARCHAR(255), CHANNEL VARCHAR(255), CORRELATION_ID VARCHAR(255), CORRELATION_KEYS VARCHAR(255), CORRELATION_STATUS VARCHAR(255), CREATE_TIME TIMESTAMP, DIRECTION INTEGER, EPR CLOB, FAULT VARCHAR(255), FAULT_EXPLANATION VARCHAR(255), OPERATION VARCHAR(255), PARTNER_LINK_MODEL_ID INTEGER, PATTERN VARCHAR(255), PIPED_ID VARCHAR(255), PORT_TYPE VARCHAR(255), PROPAGATE_TRANS BIT, STATUS VARCHAR(255), SUBSCRIBER_COUNT INTEGER, CORR_ID BIGINT, PARTNER_LINK_ID BIGINT, PROCESS_ID BIGINT, PROCESS_INSTANCE_ID BIGINT, REQUEST_MESSAGE_ID BIGINT, RESPONSE_MESSAGE_ID BIGINT, PRIMARY KEY (MESSAGE_EXCHANGE_ID), FOREIGN KEY (PROCESS_INSTANCE_ID) REFERENCES ODE_PROCESS_INSTANCE(ID) ON DELETE CASCADE, FOREIGN KEY (PARTNER_LINK_ID) REFERENCES ODE_PARTNER_LINK (PARTNER_LINK_ID) ON DELETE CASCADE, FOREIGN KEY (PROCESS_ID) REFERENCES ODE_PROCESS (ID) ON DELETE CASCADE, FOREIGN KEY (CORR_ID) REFERENCES ODE_CORRELATOR (CORRELATOR_ID) ON DELETE CASCADE);
CREATE TABLE ODE_MESSAGE (MESSAGE_ID BIGINT NOT NULL, DATA CLOB, HEADER CLOB, TYPE VARCHAR(255), MESSAGE_EXCHANGE_ID VARCHAR(255), PRIMARY KEY (MESSAGE_ID), FOREIGN KEY (MESSAGE_EXCHANGE_ID) REFERENCES ODE_MESSAGE_EXCHANGE (MESSAGE_EXCHANGE_ID) ON DELETE CASCADE);
...
CREATE TABLE ODE_MESSAGE_ROUTE (MESSAGE_ROUTE_ID BIGINT NOT NULL, CORRELATION_KEY VARCHAR(255), GROUP_ID VARCHAR(255), ROUTE_INDEX INTEGER, PROCESS_INSTANCE_ID BIGINT, ROUTE_POLICY VARCHAR(16), CORR_ID BIGINT, PRIMARY KEY (MESSAGE_ROUTE_ID), FOREIGN KEY (PROCESS_INSTANCE_ID) REFERENCES ODE_PROCESS_INSTANCE(ID) ON DELETE CASCADE);
...
CREATE TABLE HT_MESSAGE (MESSAGE_ID BIGINT NOT NULL, MESSAGE_DATA CLOB, MESSAGE_HEADER CLOB, MESSAGE_TYPE VARCHAR(255), MESSAGE_NAME VARCHAR(512), TASK_ID BIGINT, PRIMARY KEY (MESSAGE_ID));
...
CREATE INDEX I_DEDLINE_TASK ON HT_DEADLINE (TASK_ID);
CREATE INDEX I_EVENT_TASK ON HT_EVENT (TASK_ID);
CREATE INDEX I_GNRC_RL_TASK ON HT_GENERIC_HUMAN_ROLE (TASK_ID);
CREATE INDEX I_HMNRTTY_ELEMENT ON HT_HUMANROLE_ORGENTITY (ORGENTITY_ID);
CREATE INDEX I_HMNRTTY_HUMANROLE_ID ON HT_HUMANROLE_ORGENTITY (HUMANROLE_ID);
CREATE INDEX I_MESSAGE_TASK ON HT_MESSAGE (TASK_ID);
CREATE INDEX I_PRSNMNT_DTYPE ON HT_PRESENTATION_ELEMENT (PE_TYPE);
CREATE INDEX I_PRSNMNT_TASK ON HT_PRESENTATION_ELEMENT (TASK_ID);
CREATE INDEX I_PRSNPRM_TASK ON HT_PRESENTATION_PARAM (TASK_ID);
CREATE INDEX I_TASK_FAILUREMESSAGE ON HT_TASK (FAILURE_MESSAGE);
CREATE INDEX I_TASK_INPUTMESSAGE ON HT_TASK (INPUT_MESSAGE);
CREATE INDEX I_TASK_OUTPUTMESSAGE ON HT_TASK (OUTPUT_MESSAGE);
CREATE INDEX I_TASK_PARENTTASK ON HT_TASK (PARENTTASK_ID);
CREATE INDEX I_TSK_MNT_ATTACHEDBY ON HT_TASK_ATTACHMENT (ATTACHED_BY);
CREATE INDEX I_TSK_MNT_TASK ON HT_TASK_ATTACHMENT (TASK_ID);
CREATE INDEX I_TSK_MNT_TASK1 ON HT_TASK_COMMENT (TASK_ID);
