...
CREATE TABLE ODE_SCOPE (SCOPE_ID BIGINT NOT NULL, MODEL_ID INTEGER, SCOPE_NAME VARCHAR(255), SCOPE_STATE VARCHAR(255), PROCESS_INSTANCE_ID BIGINT, PARENT_SCOPE_ID BIGINT, PRIMARY KEY (SCOPE_ID), FOREIGN KEY (PROCESS_INSTANCE_ID) REFERENCES ODE_PROCESS_INSTANCE(ID) ON DELETE CASCADE)ENGINE=innodb;
...
CREATE TABLE ODE_CORRELATION_SET (CORRELATION_SET_ID BIGINT NOT NULL, CORRELATION_KEY VARCHAR(255), NAME VARCHAR(255), SCOPE_ID BIGINT, PRIMARY KEY (CORRELATION_SET_ID), FOREIGN KEY (SCOPE_ID) REFERENCES ODE_SCOPE (SCOPE_ID) ON DELETE CASCADE)ENGINE=innodb;
CREATE TABLE ODE_CORRELATOR (CORRELATOR_ID BIGINT NOT NULL, CORRELATOR_KEY VARCHAR(255), PROC_ID BIGINT, PRIMARY KEY (CORRELATOR_ID), FOREIGN KEY (PROC_ID) REFERENCES ODE_PROCESS (ID) ON DELETE CASCADE)ENGINE=innodb;
CREATE TABLE ODE_CORSET_PROP (ID BIGINT NOT NULL, CORRSET_ID BIGINT, PROP_KEY VARCHAR(255), PROP_VALUE VARCHAR(255), PRIMARY KEY (ID), FOREIGN KEY (CORRSET_ID) REFERENCES ODE_CORRELATION_SET (CORRELATION_SET_ID) ON DELETE CASCADE)ENGINE=innodb;
CREATE TABLE ODE_EVENT (EVENT_ID BIGINT NOT NULL, DETAIL VARCHAR(255), DATA BLOB, SCOPE_ID BIGINT, TSTAMP DATETIME, TYPE VARCHAR(255), INSTANCE_ID BIGINT, PROCESS_ID BIGINT, PRIMARY KEY (EVENT_ID), FOREIGN KEY (INSTANCE_ID) REFERENCES ODE_PROCESS_INSTANCE(ID) ON DELETE CASCADE, FOREIGN KEY (SCOPE_ID) REFERENCES ODE_SCOPE (SCOPE_ID) ON DELETE CASCADE, FOREIGN KEY (PROCESS_ID) REFERENCES ODE_PROCESS (ID) ON DELETE CASCADE, FOREIGN KEY (INSTANCE_ID) REFERENCES ODE_PROCESS_INSTANCE(ID) ON DELETE CASCADE)ENGINE=innodb;
...
CREATE TABLE ODE_MESSAGE_ROUTE (MESSAGE_ROUTE_ID BIGINT NOT NULL, CORRELATION_KEY VARCHAR(255), GROUP_ID VARCHAR(255), ROUTE_INDEX INTEGER, PROCESS_INSTANCE_ID BIGINT, ROUTE_POLICY VARCHAR(16), CORR_ID BIGINT,PRIMARY KEY (MESSAGE_ROUTE_ID), FOREIGN KEY (PROCESS_INSTANCE_ID) REFERENCES ODE_PROCESS_INSTANCE(ID) ON DELETE CASCADE)ENGINE=innodb;
...
CREATE TABLE HT_TASK (id BIGINT NOT NULL, ACTIVATION_TIME DATETIME, COMPLETE_BY_TIME DATETIME, CREATED_ON DATETIME, ESCALATED VARCHAR(1) NOT NULL, EXPIRATION_TIME DATETIME, TASK_NAME VARCHAR(255) NOT NULL, PRIORITY INTEGER NOT NULL, SKIPABLE VARCHAR(1) NOT NULL, START_BY_TIME DATETIME, STATUS VARCHAR(255) NOT NULL, STATUS_BEFORE_SUSPENSION VARCHAR(255), TENANT_ID INTEGER NOT NULL, TASK_TYPE VARCHAR(255) NOT NULL, UPDATED_ON DATETIME, FAILURE_MESSAGE BIGINT, INPUT_MESSAGE BIGINT, OUTPUT_MESSAGE BIGINT, PARENTTASK_ID BIGINT, PRIMARY KEY (id)) ENGINE = innodb;
