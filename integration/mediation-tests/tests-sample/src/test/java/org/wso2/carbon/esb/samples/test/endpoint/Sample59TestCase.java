 ...
 */
public class Sample59TestCase extends ESBIntegrationTest {
    ...
    public void testLoadBalanceEndpoint() throws Exception {
        ...
        for (int i = 0; i < 12; i++) {
            String response = lbClient.sendLoadBalanceRequest(getMainSequenceURL(), null);
            Assert.assertNotNull(response, "Response is null");
            Assert.assertTrue(response.contains(resMessages[i]),
                              "Response should be : " + resMessages[i]);
        }
    }
