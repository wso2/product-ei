 ...
 */
public class Sample154TestCase extends ESBSampleIntegrationTest {
    ...
    public void setEnvironment() throws Exception {
        super.init();
        loadSampleESBConfiguration(154);
    ...
    @Test(groups = { "wso2.esb" }, description = "Load Balancing with Proxy Services")
    public void testLoadBalancingProxy() throws Exception {

        String[] responseServers =
            new String[] { "Response from server: Server_1", "Response from server: Server_2",
                           "Response from server: Server_3" };

        for (int i = 0; i < 3; i++) {
            String response = lbClient.sendLoadBalanceRequest(getProxyServiceURLHttp("LBProxy"),
                                                              null);
            Assert.assertNotNull(response, "Response is null");
            Assert.assertTrue(response.contains(responseServers[i]),
                              "Response server should be : " + responseServers[i]);
        }
    }
