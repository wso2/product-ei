...
public class Sample53TestCase extends ESBIntegrationTest {
    ...
    @Test(groups = { "wso2.esb" }, description = "Using Failover Endpoints to Handle Peak Loads")
    public void testLoadBalanceEndpoint() throws Exception {
        String[] resMessages = { "Response from server: Server_1", "Response from server: Server_1",
                                 "Response from server: Server_1", "Response from server: Server_2",
                                 "Response from server: Server_2", "Response from server: Server_2",
                                 "Response from server: Server_3", "Response from server: Server_3",
        ...
        for (int i = 0; i < 10; i++) {
            ...
            try {
                ...
                Assert.assertTrue(response.contains(resMessages[i]), "Response server wrong");
            } catch (AxisFault ex) {
                assertException = ex.getMessage().contains(resMessages[i]);
            }
            ...
            if (i == 9) {
                Assert.assertTrue(assertException,
                                  "Message [COULDN'T SEND THE MESSAGE TO THE SERVER] not found");
            }
