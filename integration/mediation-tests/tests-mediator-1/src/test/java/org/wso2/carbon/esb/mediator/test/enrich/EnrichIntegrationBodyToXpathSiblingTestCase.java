...
public class EnrichIntegrationBodyToXpathSiblingTestCase extends ESBIntegrationTest {
    ...
    public void testEnrichMediator() throws Exception {
        ...
        OMElement returnTag = response.getFirstChildWithName(
                new QName("http://services.samples", "return"));

        OMElement bodySibling = (OMElement) returnTag.getNextOMSibling();
        OMElement returnTagInsideBodySibling = bodySibling.getFirstChildWithName(
                new QName("http://services.samples", "return"));
        OMElement symbolTag = returnTagInsideBodySibling.getFirstChildWithName(
