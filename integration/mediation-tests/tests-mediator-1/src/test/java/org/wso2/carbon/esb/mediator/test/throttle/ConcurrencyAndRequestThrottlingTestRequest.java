...
public class ConcurrencyAndRequestThrottlingTestRequest extends ESBIntegrationTest {

    private final int CONCURRENT_CLIENTS = 10;
    private final int AS_POLICY_ACCESS_GRANTED = 4;
    private final int AS_POLICY_ACCESS_DENIED = 6;
    private final int THROTTLE_MAX_MSG_COUNT = 4;
    //Since Max Msg Count is 4 even concurrent access is 10 it will throttle after 4
    ...
    public void setEnvironment() throws Exception {
        ...
        super.init();
        loadESBConfigurationFromClasspathIfNotExists("/artifacts/ESB/synapseconfig/throttle/ConcurrencyAndRequestBasedThrottlingRequest.xml");
    ...
    @Test(groups = "wso2.esb",
          description = "Use of both concurrency throttling and request rate based throttling- Throttle Request Massages", enabled = false)
    ...
    private void initClients() {
        for (int i = 0; i < CONCURRENT_CLIENTS; i++) {
            concurrencyAndRequestThrottleTestClients[i] = new ConcurrencyAndRequestThrottleTestClient(
                    getProxyServiceURLHttp("StockQuoteProxy"), list, clientsDone, requestThrottledClients, THROTTLE_MAX_MSG_COUNT);
        }
        for (int i = 0; i < CONCURRENT_CLIENTS; i++) {
